	...
	{{
		...
		static string ParseContent(string tweeCode, Dictionary<string,string> vars)
        {
            ...
            output = rx_IfElse.Replace(output, match => {
				...
				condition = rx_Vars.Replace(condition, varName => {
            ...
            // <<set ... >>
            output = rx_Setter.Replace(output, match => {
				return String.Format("{0};", rx_Vars.Replace(match.Groups[1].Value, varName => {
					string val = varName.Groups[1].Value;
					vars[val] = null;
					return val;
				}));
            ...
            output = rx_Link.Replace(output, match => {
                ...
                string setters = match.Groups["setters"].Length > 0 ?
					string.Format("() =>{{ {0}; }}", rx_Vars.Replace(match.Groups["setters"].Value, varName => {
						string val = varName.Groups[1].Value;
						vars[val] = null;
						return val;
					})) : // stick the setter into a lambda
                    null;
            ...
            // <<display ... >
            output = rx_Display.Replace(output, "yield return new TwineDisplay($1);");
