...
namespace UnityTwine.Editor.Parsers
{
	public class SugarCubeParser : TwineParser
	{
		...
		static Regex rx_PassageBody = new Regex(@"
				...
				(?'link'
					(?>(?'open'\[\[))
						(((?'linkName'[^|\n]+?)\s*=\s*)?(?'linkText'.*?)\|)?(?'linkPassage'.+?)(\]\[(?'linkSetter'.*?))?
					(?>(?'-open'\]\]))
				)
		...
		public override string ParsePassageBody(string passageBody)
		{
			this._indent = 0;

			var textBuffer = new StringBuilder();
			var outputBuffer = new StringBuilder();
			MatchCollection matches = rx_PassageBody.Matches(passageBody);
			...
			foreach (Match match in matches)
			{
				...
				else if (match.Groups["link"].Success)
				{
					...
					string passage = match.Groups["linkPassage"].Value;

					bool hasText = match.Groups["linkText"].Success;
					string text = hasText ? match.Groups["linkText"].Value : passage;
					//if (hasText)
					//{
